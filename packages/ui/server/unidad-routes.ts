...
import { extractText } from '@mycolegal-app/text-extract';
...
export function createUnidadRoutes(deps: UnidadDeps) {
  ...
  const resolveOcr = async (
    ...
    try {
      ...
      // import dinámico: si la app va sin @mycolegal-app/text-extract instalado, esto
      // falla y se captura → null (la UR sigue funcionando, solo sin OCR). Evita
      // acoplar el bump de ui al del package de extracción para no romper consumidores.
      const { ocrViaPlatform } = await import('@mycolegal-app/text-extract');
